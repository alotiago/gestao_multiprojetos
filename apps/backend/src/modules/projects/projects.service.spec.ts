...
import { CreateReceitaDto } from './dto/receita.dto';
...
describe('ProjectsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('deve aplicar filtro de status', async () => {
      ...
      await service.findAll({ status: ProjectStatus.ENCERRADO });

      expect(prisma.project.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ status: ProjectStatus.ENCERRADO }),
        }),
      );
    });
  ...
  describe('update', () => {
    it('deve atualizar projeto com sucesso', async () => {
      const updateDto: UpdateProjectDto = { nome: 'Projeto Atualizado' };
  ...
  describe('delete', () => {
    it('deve fazer soft delete do projeto', async () => {
      ...
      expect(result.project.status).toBe(ProjectStatus.ENCERRADO);
    });
  ...
  describe('createReceita', () => {
    const receitaDto: CreateReceitaDto = {
      mes: 3,
      ano: 2026,
      tipoReceita: 'serviço',
    ...
    it('deve lançar NotFoundException se projeto não existe', async () => {
      ...
      await expect(service.createReceita('inexistente', receitaDto)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('deve lançar ConflictException se receita duplicada', async () => {
      ...
      await expect(service.createReceita('proj-1', receitaDto)).rejects.toThrow(ConflictException);
    });
  ...
  describe('calcularFcst', () => {
    it('deve lançar BadRequestException para anoFim > 2035', async () => {
      ...
      await expect(service.calcularFcst('proj-1', 2036)).rejects.toThrow(BadRequestException);
    });
