...
const ENCARGOS_TRABALHISTAS: Record<string, number> = {
...
export class SindicatoService {
  ...
  async findAll(filters: FilterSindicatoDto) {
    const where: any = {};
  ...
  async create(dto: CreateSindicatoDto) {
    ...
    if (exists) throw new ConflictException(`Sindicato '${dto.nome}' já cadastrado`);
  ...
  async delete(id: string) {
    ...
    if (count > 0) {
      throw new ConflictException(
        `Não é possível excluir sindicato com ${count} colaboradores vinculados ativos`,
      );
    }
  ...
  async aplicarDissidio(dto: AplicarDissidioDto) {
    ...
    await this.prisma.sindicato.update({
      ...
      data: {
        percentualDissidio: new Decimal(percentualReajuste),
  ...
  async simularImpactoFinanceiro(dto: SimulacaoTrabalhistaDto) {
    const sindicato = await this.findById(dto.sindicatoId);
    const { salarioBase } = dto;
    const cargaHoraria = dto.cargaHorariaMensal || 176; // 8h × 22 dias

    // Calcular encargos
    const encargos: Array<{ tipo: string; percentual: number; valor: number }> = [];
    let totalEncargos = 0;
  ...
  async relatorioPorRegiao() {
    ...
    const regioes = sindicatos.reduce<
      Record<string, { sindicatos: number; colaboradores: number; mediaDissidio: number }>
    >((acc, s) => {
    ...
    for (const regiao of Object.keys(regioes)) {
      if (regioes[regiao].sindicatos > 0) {
        regioes[regiao].mediaDissidio = Math.round(
          (regioes[regiao].mediaDissidio / regioes[regiao].sindicatos) * 10000,
        ) / 10000;
      }
    }
