...
import { CalendarioService } from '../modules/calendario/calendario.service';
...
describe('Integração: Calendário → Operations', () => {
  ...
  function setupCascataMocks(colabs = [colaboradorJoao]) {
    jest.spyOn(calendarioService, 'calcularDiasUteis').mockResolvedValue(mockDiasUteis as any);
  ...
  it('recálculo cascata deve chamar calcularDiasUteis do CalendarioService', async () => {
    ...
    expect(calendarioService.calcularDiasUteis).toHaveBeenCalledWith(
      expect.objectContaining({ mes: 3, ano: 2026, estado: 'SP' }),
    );
