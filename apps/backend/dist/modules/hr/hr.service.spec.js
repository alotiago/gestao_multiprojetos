...
describe('HrService', () => {
    ...
    describe('findAll', () => {
        ...
        it('deve filtrar por status', async () => {
            ...
            await service.findAll({ status: client_1.UserStatus.DESLIGADO });
    ...
    describe('create', () => {
        ...
        it('deve lançar NotFoundException se sindicato não existe', async () => {
            ...
            await expect(service.create(dtoComSindicato)).rejects.toThrow(common_1.NotFoundException);
        });
    });
    // ===================== UNIT: update =====================
    describe('update', () => {
        ...
        it('deve lançar NotFoundException para colaborador inexistente', async () => {
            mockPrisma.colaborador.findFirst.mockResolvedValue(null);
            await expect(service.update('nao-existe', { cargo: 'Sênior' })).rejects.toThrow(common_1.NotFoundException);
        });
    });
    // ===================== UNIT: delete =====================
    describe('delete', () => {
        it('deve desativar colaborador (soft delete)', async () => {
            ...
            mockPrisma.colaborador.update.mockResolvedValue({
                ...mockColaborador,
                ativo: false,
                status: client_1.UserStatus.INATIVO,
            });
            const result = await service.delete('col-001');
            expect(result.ativo).toBe(false);
            expect(result.status).toBe(client_1.UserStatus.INATIVO);
        });
    });
    // ===================== UNIT: importarCSV =====================
    describe('importarCSV', () => {
        it('deve lançar BadRequestException para CSV vazio', async () => {
            await expect(service.importarCSV('')).rejects.toThrow(common_1.BadRequestException);
        });
        it('deve lançar BadRequestException para CSV sem campos obrigatórios', async () => {
            const csv = 'nome;cargo\nJoão;Dev';
            await expect(service.importarCSV(csv)).rejects.toThrow(common_1.BadRequestException);
        });
        it('deve importar CSV válido', async () => {
            const csv = 'matricula;nome;cargo;taxahora;cargahoraria;cidade;estado;dataadmissao\n' +
    ...
    describe('createJornada', () => {
        ...
        it('deve lançar ConflictException para jornada duplicada', async () => {
            ...
            await expect(service.createJornada('col-001', { mes: 1, ano: 2026, horasPrevistas: 176 })).rejects.toThrow(common_1.ConflictException);
        });
    });
    // ===================== UNIT: férias =====================
    describe('createFerias', () => {
        ...
        it('deve lançar BadRequestException se data fim < data início', async () => {
            mockPrisma.colaborador.findFirst.mockResolvedValue(mockColaborador);
            await expect(service.createFerias('col-001', {
    ...
    describe('createDesligamento', () => {
        ...
        it('deve lançar ConflictException se já possui desligamento', async () => {
            ...
            await expect(service.createDesligamento('col-001', {
                dataDesligamento: '2026-06-30',
                motivo: 'Duplicado',
