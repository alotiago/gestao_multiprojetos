...
let DashboardService = class DashboardService {
     ...
     */
    async getDashboardExecutivo(ano, projectId) {
        ...
        const [projetos, receitas, custosMensais, despesas, impostos, colaboradores, jornadas] = await Promise.all([
        ...
        const totalCustoPessoal = custosMensais.reduce((s, c) => s + Number(c.custoFixo) + Number(c.custoVariavel), 0);
        ...
        // Projetos por status
        const statusDistribuicao = projetos.reduce((acc, p) => {
     ...
     */
    async getDashboardFinanceiro(ano, mes, projectId) {
        ...
        const projetoMap = projetos.reduce((acc, p) => {
            acc[p.id] = { id: p.id, nome: p.nome, cliente: p.cliente, status: p.status };
            return acc;
        }, {});
        // Agrupa por projeto
        const porProjeto = {};
        const getOrCreate = (projectId) => {
        ...
        const resultado = Object.values(porProjeto).map((p) => {
     ...
     */
    async exportDashboardFinanceiroCsv(ano, mes, projectId) {
    ...
    // =============================================================
    async getDashboardRecursos(ano, mes) {
    ...
    // =============================================================
    async getVisaoAnoAno(anoInicio, anoFim) {
        ...
        return anos.map((ano) => {
            ...
            const custoPessoal = recCustos.reduce((s, c) => s + Number(c.custoFixo) + Number(c.custoVariavel), 0);
    ...
    // =============================================================
    async getDashboardProjetos(ano) {
