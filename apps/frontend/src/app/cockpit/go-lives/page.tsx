      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-heading font-semibold text-[#0C1B3A]">
            Go-Lives Pendentes
            {pendentes.length > 0 && (
              <span className="ml-2 text-xs font-normal text-gray-500">({pendentes.length})</span>
            )}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            <p className="text-gray-500 text-sm">Nenhum go-live pendente.</p>
                  ...
                  return (
                    <tr key={gl.id} className={`hover:bg-gray-50/50 ${isOverdue ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="px-5 py-3 text-center">
                        <RowActionsMenu
                          items={[
                            { label: 'Marcar concluído', icon: '✅', tone: 'success', onClick: () => handleToggleConcluido(gl) },
                            { label: 'Editar', icon: '✏️', onClick: () => handleEdit(gl) },
                            { label: 'Remover', icon: '🗑️', tone: 'danger', onClick: () => handleDelete(gl.id) },
                          ]}
                          align="left"
                        />
                      </td>
      ...
      {concluidos.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-base font-heading font-semibold text-[#0C1B3A]">
              Go-Lives Concluídos
              <span className="ml-2 text-xs font-normal text-gray-500">({concluidos.length})</span>
            </h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
